    ...
    const hit = await Promise.race([
      page.evaluate(() => {
        ...
        if (timeEl) {
          ...
          if (dt) return { raw: dt, via: 'time-tag' };
          const txt = timeEl.textContent && timeEl.textContent.trim();
          if (txt) return { raw: txt, via: 'time-tag' };
        }
      ...
      new Promise((_, reject) => setTimeout(() => reject(new Error('extract-timeout')), timeoutMs)),
    ]);
